   ...
   <div class="table-responsive action_table dataTables_wrapper">
      <table id="" class="display dataTable responsive nowrap" cellspacing="0" width="100%">
         <thead>
            ...
            if(!isset($_SESSION['cases_tomorrow_pagination_page'])){
              $_SESSION['cases_tomorrow_pagination_page'] = 1;
            }
            ...
            $qry_cases = "SELECT lawyer_id,case_id, next_date, judge_name, stage, court_number, case_number, party_a, party_b, court_name, client_name, category, file_no, client_phone, client_email FROM tbl_cases WHERE (".$cases_id."next_date = '".date('Y-m-d', strtotime('+1 day'))."') AND lawyer_id IN (".$lawyer_id.") ".$string_query." ORDER BY CASE 
            ...
            $get_total_rows_tomorrow = mysqli_num_rows($select_cases); 
            $pages_tomorrow = ceil($get_total_rows_tomorrow/$item_per_page);
            ?>
            <input type="hidden" id="total_cases_available_tomorrow" value="<?php echo $get_total_rows_tomorrow; ?>"></input>
