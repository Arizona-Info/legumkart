...
$position = (($page_number-1) * $item_per_page);
   ...
   $srl =0;
   $counselcases = mysqli_query($conn,"SELECT t1.*, t2.* FROM tbl_counsel_cases t1, tbl_cases t2 WHERE cc_date = '".date('Y-m-d')."' AND t1.cc_case_id=t2.case_id AND t1.cc_status='Accepted' AND t1.counsel_id = '".$_SESSION['user_id']."'".$string_query." LIMIT $position, $item_per_page");
...
?>    
  <tr>
    ...
    <?php $srl++; } 
        echo "<input type='hidden' class='NumberOfCasesToday' value = 'Showing ".((($page_number - 1)*10)+1)." To ".((($page_number - 1)*10)+$srl)." Of '>";
    }
    else{
      ...
      echo "<input type='hidden' class='NumberOfCasesToday' value = 'Showing 0 To 0 Of '>";
    }
