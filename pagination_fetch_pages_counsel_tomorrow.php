...
if(isset($_POST['page'])){
  $_SESSION['upcoming_counsel_pagination_page2'] = filter_var($_POST["page"], FILTER_SANITIZE_NUMBER_INT, FILTER_FLAG_STRIP_HIGH);
  $page_number = $_SESSION['upcoming_counsel_pagination_page2'];
...
$srl =0;
 ...
 $counselcases = mysqli_query($conn,"SELECT t1.*, t2.* FROM tbl_counsel_cases t1, tbl_cases t2 WHERE cc_date = '".date('Y-m-d', strtotime('+1 day'))."' AND t1.cc_case_id=t2.case_id AND t1.cc_status='Accepted' AND t1.counsel_id = '".$_SESSION['user_id']."'".$string_query." LIMIT $position, $item_per_page");
...
<td>
<?php if($counselcasesrow['cc_bill_pdf'] == ""){ ?>
<u><a href="#" onclick="generateBillBtn(<?php echo "'".$counselcasesrow['cc_id']."','".$counsellawyerrow['lawyer_id']."','".$counselcasesrow['cc_type']."','".$counsellawyerrow['firm_name']."'"; ?>)" title="Court Details" <?php if($row['cc_action']=="Not Attended") { echo 'style="display: none;"'; } ?> id="visible_2<?php echo $counselcasesrow['cc_id'];?>">Generate Bill</a></u>
...
<?php } ?>
  ...
  <?php $srl++; } 
      echo "<input type='hidden' class='NumberOfCasesTomorrow' value = 'Showing ".((($page_number - 1)*10)+1)." To ".((($page_number - 1)*10)+$srl)." Of '>";
  }
  else{
    ...
    echo "<input type='hidden' class='NumberOfCasesTomorrow' value = 'Showing 0 To 0 Of '>";
  }
